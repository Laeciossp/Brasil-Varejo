...
// --- ALTERADO APENAS O ID DA CATEGORIA (PIJAMAS) ---
const ID_CATEGORIA = '9bcd8ecd-39e1-4e9d-88f1-983120e621ae';
...
async function startSheinImport() {
    console.log('🚀 Iniciando IMPORTADOR PIJAMAS (Clone Fiel)...');
    
    let cookies = [];
    try { cookies = JSON.parse(fs.readFileSync('cookies.json', 'utf-8')); } catch (e) {}
    let links = [];
    try {
        // --- ALTERADO O ARQUIVO DE LEITURA PARA LINKS-PIJAMAS.TXT ---
        const fileContent = fs.readFileSync('links-pijamas.txt', 'utf-8');
        links = fileContent.split('\n').map(l => l.trim()).filter(l => l.startsWith('http'));
        console.log(`\n📄 Lendo arquivo: links-pijamas.txt (${links.length} links)`);
    } catch (e) { console.log('❌ Erro: Crie o arquivo links-pijamas.txt'); return; }
