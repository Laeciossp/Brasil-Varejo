...
// V11: Separação Real de Conjuntos (Shorts vs Calças) com ID novo
...
const CATEGORY_MAP = [
    // === PRIORIDADE 1: CONJUNTOS (Verificação no Link e Texto) ===
    ...
    // 1.2 Conjuntos SHORTS (ID NOVO QUE VOCÊ ENVIOU AGORA)
    {
    ...
    // 1.3 Conjuntos CALÇAS (ID ANTIGO - Fica exclusivo para calças)
    {
        id: '25aa9265-d05b-4fc0-be64-8f41a20b780a',
        rules: [
            /CONJUNTO.*CAL(Ç|C)A/i, 
            /CJ.*CAL(Ç|C)A/i,
...
const DEFAULT_CATEGORY_ID = '7ef4bb1b-a674-41cc-b38e-dd3daa2f19ac'; // Padrão: Blusas

// --- FUNÇÃO DE MAPEAMENTO INTELIGENTE (Com URL) ---
function getFixedCategoryId(detectedName, productTitle, productDescription, productUrl) {
    const cleanUrl = productUrl ?
productUrl.replace(/-/g, ' ').replace(/\//g, ' ') : '';
...
let detectedCategory = '';
        ...
        if (breadcrumbs.length > 0) {
            const validCrumbs = breadcrumbs.map(li => li.innerText.trim())
                .filter(t => !t.includes('Início') && !t.includes('Home') && t !== '/');
if (validCrumbs.length > 0) detectedCategory = validCrumbs[validCrumbs.length - 1]; 
    ...
    // --- LÓGICA DE CATEGORIA (Passando a URL) ---
...
console.log(`   📦 ${mainData.title}`);
    console.log(`   🔗 Link analisado para categoria.`);
console.log(`   📂 ID Categoria Definido: ${targetCategoryId}`);

    const categoryRef = [{ _type: 'reference', _ref: targetCategoryId, _key: generateKey() }];
console.log(`   ⬆️ Uploading imagens...`);
...
if (mainImageAssets.length > 0) {
    ...
    if (mainData.variantLinks.length > 0) {
        console.log(`   🔄 Visitando variantes...`);
...
const variantPageData = await page.evaluate(() => {
                    ...
                    const tds = Array.from(document.querySelectorAll('#product-attribute-specs-table tr'));
                    const colorRow = 
tds.find(tr => tr.innerText.includes('Cor'));
...
console.log(`   ✅ SUCESSO! Criado ID: ${res._id}`);
    } catch (err) { 
...
async function startMassScraper() {
  console.log('🚀 Iniciando Importador V11 (Separação Total de Conjuntos)...');
  let links = [];
try {
