...
export default function ProductDetails() {
  ...
  const handleCalculateShipping = async (cepOverride) => {
    ...
    try {
      ...
      if (Array.isArray(rawOptions) && rawOptions.length > 0) {
          ...
          if (isLocal) {
             // LOGICA LOCAL: PEGA O MAIS BARATO E CHAMA DE EXPRESSO PALASTORE
             const cheapest = candidates[0];
             finalOptions.push({
                name: "Expresso Palastore ⚡",
                price: cheapest.price, 
                delivery_time: 1, // Entrega rápida local
                company: "Própria"
             });
          } else {
             // LOGICA NACIONAL: FILTRA SÓ UM PAC E UM SEDEX
             ...
             if (bestEconomy) {
                finalOptions.push({
                    ...
                    delivery_time: bestEconomy.days + handlingDays, // SOMA DO MANUSEIO
             ...
             if (bestExpress && bestExpress.name !== bestEconomy?.name) {
                finalOptions.push({
                    ...
                    delivery_time: bestExpress.days + handlingDays, // SOMA DO MANUSEIO
                    company: "Correios/Jadlog"
                });
             }
          }
  ...
  const createCartItem = () => {
      ...
      return {
        ...
        handlingTime: handlingDays, // ENVIO
        width: product.logistics?.width || 15,
        height: product.logistics?.height || 15,
        length: product.logistics?.length || 15,
        weight: product.logistics?.weight || 0.5
