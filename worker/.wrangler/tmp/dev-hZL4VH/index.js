...
// .wrangler/tmp/bundle-KPeiuz/checked-fetch.js
...
// .wrangler/tmp/bundle-KPeiuz/strip-cf-connecting-ip-header.js
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (url.pathname === "/callback/melhorenvio") {
        ...
        return new Response(JSON.stringify({ error: "Erro na autoriza\xE7\xE3o", details: data }), { status: 400, headers });
      }
      if (req.method === "POST" && url.pathname.includes("shipping")) {
        ...
        if (productId && env.SANITY_PROJECT_ID) {
          try {
            ...
            console.log("Erro Sanity Frete:", e);
          }
        }
        let handlingTime = 0;
        ...
        if (isFreeShipping) {
          return new Response(JSON.stringify([{
            name: "FRETE GR\xC1TIS",
            price: 0,
            delivery_time: 12,
            delivery_range: { min: 5, max: 12 },
            company: { picture: null },
            custom_msg: "Promo\xE7\xE3o Brasil Varejo"
          }]), { headers });
        }
        if (!token)
      ...
      if (req.method === "POST" && url.pathname.includes("checkout")) {
        ...
        const newOrder = {
          _type: "order",
          orderNumber: `PALA-${Math.floor(Date.now() / 1e3)}`,
          status: "pending",
        ...
        if (!orderId) {
          throw new Error("Falha cr\xEDtica ao criar pedido no banco de dados.");
        }
        const isCashPayment = tipoPagamento === "pix" || tipoPagamento === "boleto";
        const fatorDesconto = isCashPayment ? 0.9 : 1;
        ...
        const preferenceData = {
          ...
          // Link com o ID do Sanity que acabamos de criar
          ...
          expires: true,
          date_of_expiration: new Date(Date.now() + 3 * 24 * 60 * 60 * 1e3).toISOString()
        };
...
// .wrangler/tmp/bundle-KPeiuz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KPeiuz/middleware-loader.entry.ts
