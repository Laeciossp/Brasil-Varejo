...
// --- ALTERADO APENAS O ID DA CATEGORIA (JEANS) ---
const ID_CATEGORIA = '03264276-eb55-4412-8065-5552ba913b42';
const generateKey = () => Math.random().toString(36).substring(2, 15);
const LOGISTICA_PADRAO = { weight: 0.5, width: 25, height: 5, length: 25 }; // Ajustei levemente peso para Jeans
...
async function startSheinImport() {
    console.log('🚀 Iniciando IMPORTADOR JEANS (Clone Fiel)...');
    
    let cookies = [];
    try { cookies = JSON.parse(fs.readFileSync('cookies.json', 'utf-8')); } catch (e) {}
    let links = [];
    try {
        // --- ALTERADO O ARQUIVO DE LEITURA PARA LINKS-JEANS.TXT ---
        const fileContent = fs.readFileSync('links-jeans.txt', 'utf-8');
        links = fileContent.split('\n').map(l => l.trim()).filter(l => l.startsWith('http'));
        console.log(`\n📄 Lendo arquivo: links-jeans.txt (${links.length} links)`);
    } catch (e) { console.log('❌ Erro: Crie o arquivo links-jeans.txt'); return; }
    ...
    for (const [index, link] of links.entries()) {
        ...
        try {
            ...
            const doc = {
                ...
                // --- CATEGORIA JEANS ---
                categories: [{ _key: generateKey(), _type: 'reference', _ref: ID_CATEGORIA }],
                ...
                productType: 'fashion',
                fashionSpecs: { gender: 'Fem', material: 'Jeans', model: 'Calça' }
            };
