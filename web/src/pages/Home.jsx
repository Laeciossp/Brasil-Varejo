...
import { Package, ChevronLeft, ChevronRight } from 'lucide-react'; // Ícones necessários
          ...
          const MediaContent = (
            <>
              {slide.mediaType === 'video' && slide.videoUrl ? (
                ...
                <img src={urlFor(slide.image)} alt={slide.title} className="w-full h-full object-cover object-top" />
              )}
            </>
          );
          return (
            <div key={idx} className="min-w-full h-full relative">
              {slide.link ? (
                <a href={slide.link} className="block w-full h-full cursor-pointer">{MediaContent}</a>
              ) : (
                <div className="w-full h-full">{MediaContent}</div>
              )}
            </div>
          );
        })}
...
// --- Bloco D: Carrossel de Produtos (Híbrido + Link Corrigido) ---
const ProductCarouselBlock = ({ data }) => {
  ...
  // Lógica: Se for automático (categoria), cria o link para /category/slug.
  // Se for manual, manda para a loja geral (/products).
    ...
    <div className="max-w-[1440px] mx-auto my-10 px-4">
      <div className="flex justify-between items-center mb-4 border-b pb-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to={seeAllLink} className="text-purple-600 text-sm hover:underline font-semibold">
            Ver todos
          </Link>
          
          {/* Setas (Visíveis apenas em Desktop) */}
          <div className="hidden md:flex gap-2">
             <button onClick={() => scrollCarousel('left')} className="p-1.5 rounded-full border border-gray-200 hover:bg-gray-100 text-gray-600 transition-colors">
               <ChevronLeft size={20}/>
             </button>
             <button onClick={() => scrollCarousel('right')} className="p-1.5 rounded-full border border-gray-200 hover:bg-gray-100 text-gray-600 transition-colors">
          ...
          >
            ...
            <div className="mt-auto pt-2 border-t border-gray-50">
               {prod.oldPrice > prod.price && (
                  <span className="text-[10px] text-gray-400 line-through block mb-0.5">de {formatCurrency(prod.oldPrice)}</span>
               )}
...
export default function Home() {
  ...
  useEffect(() => {
    // ATENÇÃO: Adicionamos "categorySlug": selectedCategory->slug.current para consertar o link "Ver todos"
