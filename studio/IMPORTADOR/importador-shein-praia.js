...
async function scrapeSheinData(page, url) {
    ...
    while (true) {
        try {
            ...
            // -----------------------------------------------------------
            // 1. DADOS BÁSICOS + COR V71 (INJETADO AQUI)
            // -----------------------------------------------------------
            const basicData = await page.evaluate(async () => {
                ...
                if (title) {
                    ['Flash Sale', 'Novo', 'Top', 'Shein', 'SHEIN', 'Brasil'].forEach(w => title = title.replace(new RegExp(w, 'gi'), ''));
            ...
            // -----------------------------------------------------------
            // (Substitui o loop simples do Tabela OK por este que verifica 'Esgotado')
            ...
            const sizesFinal = await page.evaluate(async () => {
                ...
                for (const btn of buttons) {
                    ...
                    // Clique para garantir (corrige o problema do GGG esgotado)
                    try {
                        btn.click();
                        await wait(400); 
...
async function startSheinImport() {
    console.log('🚀 Iniciando BASE_TABELA_OK + COR + ESTOQUE_SEGURO...');
    ...
    for (const [index, link] of links.entries()) {
        ...
        try {
            ...
            if (data.tableText && data.tableText.length > 10) {
                ...
                console.log(`   ⚠️ Tabela VAZIA (Mesmo com lógica Tabela OK).`);
            }
